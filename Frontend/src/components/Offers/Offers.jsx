...
const Offers = () => {
  ...
  const handleClick = () => {
    navigate(`/active-offers?restaurantId=${restaurantId}`);
  };
  ...
  return (
    ...
    >
      <div className="relative w-full max-w-md bg-gradient-to-r from-gray-700 to-gray-500 text-white rounded-xl shadow-xl p-6 flex items-center justify-between hover:scale-105 transition-transform duration-300">
        <div>
          <h2 className="text-lg font-bold mb-2 animate-pulse text-yellow-500">Valentine's Special</h2>
          <p className="text-sm animate-bounce">Check out the hottest offers for you!</p>
        </div>
        <div className="text-xl font-semibold animate-ping"> &rarr;</div>
        <div
